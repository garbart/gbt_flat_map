...
namespace gbt
{
	...
	private:
		...
		static constexpr size_type funnel_bucket_size = 1;
		static_assert(default_max_load_factor > 0.0f, "gbt::funnel_flat_map max load factor must be positive");
		static_assert(funnel_bucket_size > 0, "gbt::funnel_flat_map bucket size must be positive");
		...
		bool for_each_probe(size_type hash, Callback&& callback) const
		{
			...
			while (level_width > 0)
			{
				if constexpr (funnel_bucket_size == 1)
				{
				...
				{
					const size_type bucket_width = std::min(funnel_bucket_size, level_width);
					const size_type bucket_count = level_width / bucket_width;
					const size_type bucket_index = bucket_count == 1 ? 0 : ((hash >> hash_shift) & (bucket_count - 1));
					const size_type bucket_begin = offset + bucket_index * bucket_width;

					for (size_type i = 0; i < bucket_width; ++i)
					{
						if (!callback(bucket_begin + i))
						{
							return false;
						}
					}
	...
	public:
		...
		public:
			...
			using difference_type = typename funnel_flat_map::difference_type;
			using value_type = typename funnel_flat_map::value_type;
			using pointer = typename funnel_flat_map::pointer;
			using reference = typename funnel_flat_map::reference;
		...
		private:
			iterator(funnel_flat_map* map, size_type index) : map_(map), index_(index)
			{
				advance_to_occupied();
			}

			iterator(funnel_flat_map* map, size_type index, unchecked_tag) : map_(map), index_(index)
			{
			}
		...
		public:
			...
			using difference_type = typename funnel_flat_map::difference_type;
			using value_type = typename funnel_flat_map::value_type;
			using pointer = typename funnel_flat_map::const_pointer;
			using reference = typename funnel_flat_map::const_reference;
		...
		private:
			const_iterator(const funnel_flat_map* map, size_type index) : map_(map), index_(index)
			{
				advance_to_occupied();
			}

			const_iterator(const funnel_flat_map* map, size_type index, unchecked_tag) : map_(map), index_(index)
			{
			}
			...
			const funnel_flat_map* map_ = nullptr;
			size_type index_ = 0;
		};

		funnel_flat_map() : slots_(default_initial_capacity)
		{
		}

		explicit funnel_flat_map(size_type bucket_count, const hasher& hash = hasher{}, const key_equal& equal = key_equal{})
			: slots_(normalize_capacity(bucket_count)), hash_(hash), equal_(equal)
		{
		}

		template <typename InputIt>
		funnel_flat_map(InputIt first, InputIt last) : funnel_flat_map()
		{
			insert(first, last);
		}

		funnel_flat_map(std::initializer_list<value_type> values) : funnel_flat_map()
		{
			reserve(values.size());
			insert(values.begin(), values.end());
		}

		funnel_flat_map(const funnel_flat_map&) = default;
		funnel_flat_map(funnel_flat_map&&) noexcept = default;
		funnel_flat_map& operator=(const funnel_flat_map&) = default;
		funnel_flat_map& operator=(funnel_flat_map&&) noexcept = default;
		~funnel_flat_map() = default;
		...
		void max_load_factor(float factor)
		{
			if (factor <= 0.0f)
			{
				throw std::invalid_argument("gbt::funnel_flat_map max_load_factor must be positive");
			}
		...
		mapped_type& at(const key_type& key)
		{
			...
			if (it == end())
			{
				throw std::out_of_range("gbt::funnel_flat_map::at");
			}
			return it->second;
		}
		...
		void swap(funnel_flat_map& other) noexcept(std::is_nothrow_swappable_v<hasher> && std::is_nothrow_swappable_v<key_equal>)
		{
			using std::swap;
	...
	void swap(funnel_flat_map<Key, T, Hash, KeyEqual>& lhs, funnel_flat_map<Key, T, Hash, KeyEqual>& rhs) noexcept(noexcept(lhs.swap(rhs)))
	{
		lhs.swap(rhs);
	}
